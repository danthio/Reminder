...
im=Image.open("data/logout.png")
im=im.resize((30,30))
im.save("data/logout.png")
...
def can2_b1(e):
	...
	cur_d=get_cur_date()
	tm_diff=get_time_difference(str(cur_d[0])+"/"+str(cur_d[1])+"/"+str(cur_d[2]),str(sel_date[0])+"/"+str(sel_date[1])+"/"+str(sel_date[2]),1)
...
def can_b1(e):
	...
	if state=="main":
		...
		if 10+30+5+l+5<=e.x<=10+30+5+l+5+30:
			if 10<=e.y<=10+30:
		...
		if state2==1:
			...
			#cancel
			cx,cy=586.6666666666666+15, 582.5+15
			...
			cx,cy=586.6666666666666+120-15, 582.5+15
			...
			if 586.6666666666666+15<=e.x<=586.6666666666666+120-15:
			...
			cx,cy=793.3333333333334+15, 582.5+15
			...
			cx,cy=793.3333333333334+120-15, 582.5+15
			...
			if 793.3333333333334+15<=e.x<=793.3333333333334+120-15:
				if 582.5<=e.y<=582.5+29:
...
def main():
	...
	global text
	global user
	global circle2,circle3,circle4,circle5,circle6
	global can2
	...
	can.create_image(w-5-30-12,5,image=logout,anchor="nw")


	#calender

	can.create_rectangle(0,0, 500,621, fill="#111111",outline="#111111")

	
	can.create_image(10,10,image=previous,anchor="nw")
	can.create_text(10+30+5,10+15,text=str(date[-1]),font=("FreeMono",13),fill="#ffffff",anchor="w")

	l=get_text_length(can, str(date[-1]), "FreeMono", 13)
	can.create_image(10+30+5+l+5,10,image=next_,anchor="nw")


	can.create_image(10,10+30+10,image=previous,anchor="nw")
	can.create_text(10+30+5,10+15+30+10,text=months[date[0]],font=("FreeMono",13),fill="#ffffff",anchor="w")

	l=get_text_length(can, str(months[date[0]],), "FreeMono", 13)
	can.create_image(10+30+5+l+5,10+30+10,image=next_,anchor="nw")
	...
	for n_ in range(n):
		...
		if dd[-1]==date[-1] and dd[1]==date[0]:
			...
			if dd[0]==n_+1:
				...
				x,y+10, fill="#ff1f83",outline="#ff1f83")
	...
	if state2==0:
		...
		can.create_rectangle(519,39, 981,574,outline="#000000")

		can2["width"]=981-519-2
		can2["height"]=574-39-2
		...
		if tm_diff==1:
			...
			can.create_image(xx,y,image=circle2,anchor="nw")
			can.create_image(xx+sz-30,y,image=circle2,anchor="nw")
			can.create_rectangle(xx+15,y, xx+sz-15,y+29,fill="#000000")

			can.create_text(xx+sz/2,y+15,text="Modify",font=("FreeMono",13),fill="#ffffff")
		...
		if len(rem_ar)==0:
			...
			can2.create_text(int(can2["width"])/2,int(can2["height"])/2,text="No Record",font=("FreeMono",20),fill="#000000")
			...
			for i in rem_ar:
				...
				for _ in range(len(val)):
					...
					try:
						...
						if get_text_length(can2, txt2+val[_+1]+" ", "FreeMono", 13)<=sz:
							txt+=val[_]+" "
							txt2+=val[_]+" "
						else:
							txt+="\n"+val[_]+" "
				...
				can2.create_text(25,y,text=txt,font=("FreeMono",13),anchor="nw",fill="#000000")
				...
				rem__.append([i[0],y1,y2])


				if i[1]==1:

					can2.create_image(int(can2["width"])-10-25,y-12.5-12,image=checked,anchor="nw")
				...
				can.create_image(981-30+3,574+((621-574)-30)/2,image=delete,anchor="nw")
		...
		can.create_text(xx+sz/2,y+15,text="Cancel",font=("FreeMono",13),fill="#ffffff")



		xx=(500-sz*2)/3*2+sz+500

		can.create_image(xx,y,image=circle2,anchor="nw")
		can.create_image(xx+sz-30,y,image=circle2,anchor="nw")
		can.create_rectangle(xx+15,y, xx+sz-15,y+29,fill="#000000")

		can.create_text(xx+sz/2,y+15,text="Create",font=("FreeMono",13),fill="#ffffff")
...
def create_account_():
	...
	can.create_polygon(x+15,y, x+xx-15,y, x+xx,y+15, x+xx,y+yy-15,
		x+xx-15,y+yy, x+15,y+yy, x,y+yy-15, x,y+15,fill="#ffffff",outline="#ffffff")
...
circle,circle2,circle3,circle4,circle5,circle6=0,0,0,0,0,0
checked=0
delete=0
logout=0
def load_im():
	global previous,next_
	global cur_date1,cur_date2
	global circle,circle2,circle3,circle4,circle5,circle6
	global checked
	global delete
	global logout
...
can=tk.Canvas(width=w,height=h,bg="#ffffff",relief="flat",highlightthickness=0,border=0)
...
can3.create_text(0,10,text="Write Reminder....",fill="#000000",anchor="w",font=("FreeMono",13))
