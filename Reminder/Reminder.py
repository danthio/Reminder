...
im=Image.open("data/circle2.png")
im=im.resize((6,6))
im.save("data/circle3.png")


im=Image.open("data/cur_date2.png")
im=im.resize((30,30))
im.save("data/cur_date2.png")
...
def create_reminder():
	...
	if text.get("1.0",tk.END)!="\n":
		...
		for row in rows:
			...
			if row[1]==str(sel_date[0])+"/"+str(sel_date[1])+"/"+str(sel_date[2]):
		...
		if con==0:
			...
			cur.execute("INSERT INTO reminder VALUES("+str(user)+",'"+str(sel_date[0])+"/"+str(sel_date[1])+"/"+str(sel_date[2])+"','"+str(text.get("1.0",tk.END))+"')")
			db_reminder.commit()

		else:
			cur.execute("UPDATE reminder set reminder='"+str(text.get("1.0",tk.END))+"' WHERE user_id='"+str(user)+"'")
			db_reminder.commit()
...
def main():
	...
	for n_ in range(n):
		...
		if dd[-1]==date[-1] and dd[1]==date[0]:
			...
			if dd[0]==n_+1:
				...
				can.create_rectangle(x,y, x+500/7,y+500/7,fill="#ff1f83",outline="#ff1f83")
	...
	cur.execute("SELECT * FROM reminder WHERE user_id="+str(user))
	rows=cur.fetchall()
	...
	if state2==0:
		...
		can.create_text(xx+sz/2,y+15,text="Clear",font=("FreeMono",13),fill="#ffffff")
...
circle,circle2,circle3=0,0,0
def load_im():
	global previous,next_
	global cur_date1,cur_date2
	global circle,circle2,circle3
